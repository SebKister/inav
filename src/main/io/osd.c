...
static bool osdDrawSingleElement(uint8_t item)
{
    uint16_t pos = osdLayoutsConfig()->item_pos[currentLayout][item];
     if (!OSD_VISIBLE(pos) && item != OSD_CRC && item != OSD_IMU2_CAL) {
        return false;
    }
...
case OSD_IMU2_CAL:
   ...
   //     elemPosX = 1;
    ...
    //    tfp_sprintf(&buff[0], "%d%d%d%d", secondaryImuState.calibrationStatus.acc,
   //                 secondaryImuState.calibrationStatus.gyr,
   //                 secondaryImuState.calibrationStatus.mag,
   //                 secondaryImuState.calibrationStatus.sys);
   //     buff[4] = '\0';
        break;
    }
...
void osdDrawNextElement(void)
{
    ...
    // Draw artificial horizon + tracking telemtry last
        ...
        if (secondaryImuState.calibrationStatus.acc +
            secondaryImuState.calibrationStatus.gyr +
            secondaryImuState.calibrationStatus.mag +
            secondaryImuState.calibrationStatus.sys != 12)
            osdDrawSingleElement(OSD_IMU2_CAL);
